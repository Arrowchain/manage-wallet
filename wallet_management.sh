...
echo -e '\n Arrow wallet reset script v0.2b (c) '${MAGENTA}mrlynch${RESET}, 2019
echo    '===================================================='
...
if $(pgrep -af "arrowd" > /dev/null); then
  echo -e "[$(timestamp)] - ${GREEN}Arrow daemon is running!${RESET}"
else
  echo -e "[$(timestamp)] ${RED}[ERROR]: Arrow daemon is not running. Start daemon and try again${RESET}\n"
  exit 0
fi
